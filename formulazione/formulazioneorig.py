...
def create_params(NF,NC,ND,NV,disdur):
    ...
    # a_matrix = {(k, l): 1 for (k, l) in itertools.product(D, V)} #modifica
...
def OP_model(params,rl_solution):
    m = Model('OP')
    m.Params.TimeLimit = 7200 #in secondi
    m.Params.Threads = 1
    #m.setParam(GRB.Param.IntFeasTol, 1e-9)
    # m.setParam(GRB.Param.FeasibilityTol, 1e-9)  # Aumenta la tolleranza di fattibilità
    # m.setParam(GRB.Param.MIPGap, 1e-9)  # Aumenta la tolleranza dell'ottimalità relativa
    ...
    of = quicksum(z[l, a, b] * t[a, b] for a in D + C + F for b in C + F + D for l in V)  #modifica
    ...
    # for k in D:
    #     for l in V:
    #         m.addConstr(quicksum(z[l, k, i] for i in C) <=  h[l, k], #modificato tolto  a_matrix[k, l] e messo 1 perchè tuuti i veicoli possono partire da qualsiasi depot
    #                         name='C_17_exit_from_depot({},{})'.format(k, l))
    # # (18)
    # for k in D:
    #     for l in V:
    #         m.addConstr(quicksum(z[l, j, k] for j in F) <=  h[l, k], #modifica uguale a 17
    ...
    for l in V:
        for a in D:
            m.addConstr(quicksum(z[l, a, b] for b in C) == h[l, a],
                        name='C_22bis_from_deposit_to_client_({},{})'.format(l, a))
    # (22tris)
    for l in V:
        for b in D:
            m.addConstr(quicksum(z[l, a, b] for a in F) == h[l, b],
                        name='C_22tris_from_deposit_to_client_({},{})'.format(l, b))
...
if __name__ == '__main__':
    ...
    # rl_solution = load_rl_solution('./percorsi_formulazione-completelinkage2409.json') #placeholder, qua va inserita una funzione che legga la soluzione
...
def save_solution_to_file(solution, filename="solution.txt"):
    with open(filename, "w") as f:
        for key, value in solution.items():
            f.write(f"{key}: {value}\n")
save_solution_to_file(solution, "solution-completelinkage-nosoluzione7200.txt")